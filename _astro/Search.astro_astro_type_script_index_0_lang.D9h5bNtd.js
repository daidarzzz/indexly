...
• `)}`),i>0&&(d.removeAttribute(`disabled`),await _(),v())}),window.toggleIndex=async function(e){let t=s.find(t=>t.id===e);t&&(t.active=!t.active,await _(),v())},window.removeIndex=async function(e){s=s.filter(t=>t.id!==e),s.length===0&&(d.setAttribute(`disabled`,`true`),d.value=``),await _(),v()};function v(){y();let e=[];s.forEach(t=>{t.active&&(e=e.concat(t.games))});let t=d.value.toLowerCase();c=t.trim()===``?e:e.filter(e=>e.title.toLowerCase().includes(t)),l=50,b()}function y(){if(s.length===0){p.innerHTML=`<p class="no-indexes">No hay índices cargados todavía.</p>`;return}p.innerHTML=s.map(e=>`
      <div class="index-pill ${e.active?`active`:`inactive`}">
        ...
        <div class="index-pill-actions">
          ...
          <button class="action-btn delete" onclick="removeIndex('${e.id}')" title="Eliminar índice">
            ✕
          </button>
        </div>
      </div>
    `).join(``)}function b(){if(s.length===0){f.innerHTML=``,h.innerHTML=``,m.style.display=`none`,d.placeholder=`Sube un JSON arriba para empezar...`;return}let e=c.length;if(d.placeholder=`Buscando entre ${e} juegos activos...`,e===0){f.innerHTML=`<p class="no-results">No se han encontrado juegos.</p>`,h.innerHTML=``,m.style.display=`none`;return}h.innerHTML=`Mostrando ${Math.min(l,e)} de ${e} resultados`,f.innerHTML=c.slice(0,l).map(e=>`
      <div class="game-card">
        <div class="game-info-container">
          <h3>${e.title}</h3>
          <p class="game-meta">
            <span class="source-badge">${e.source}</span> &bull; 
            <span>Peso:</span> ${e.fileSize}
          </p>
        </div>
        ${e.link?`<a href="${e.link}" target="_blank" class="magnet-link">Descargar</a>`:`<span class="no-magnet">Sin enlace</span>`}
      </div>
    `).join(``),l<e?m.style.display=`block`:m.style.display=`none`}d.addEventListener(`input`,()=>{v()}),m.addEventListener(`click`,()=>{l+=50,b()}),g();